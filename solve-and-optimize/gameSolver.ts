...
/**
 * HÀM MỚI (PORTED FROM PYTHON): Giải bài toán TSP (Traveling Salesman Problem) cho nhiều mục tiêu.
 * Chia bài toán lớn thành các đoạn A* nhỏ nối tiếp nhau.
 */
const solveTSP = (gameConfig: GameConfig, solutionConfig: Solution, blocklyConfig?: QuestBlocklyConfig): Solution | null => {
  const world = new GameWorld(gameConfig, solutionConfig);
  const startPos = gameConfig.players[0].start;
  const finishPos = gameConfig.finish;

  // 1. Xác định tất cả các điểm mục tiêu
  const goalPositions: { pos: Position, id: string, type: string }[] = [];
  const requiredGoals = solutionConfig.itemGoals || {};

  // Collectibles
  world.collectiblesById.forEach((c, id) => {
    if (requiredGoals[c.type]) {
      goalPositions.push({ pos: c.position, id: id, type: c.type });
    }
  });

  // Switches
  if (requiredGoals['switch']) {
    world.switchesByPos.forEach((s, key) => {
      // Chỉ thêm nếu switch đang tắt (hoặc logic phức tạp hơn nếu cần check state)
      // Ở đây ta đơn giản hóa là cần đi qua switch
      const [x, y, z] = key.split(',').map(Number);
      goalPositions.push({ pos: { x, y, z }, id: s.id, type: 'switch' });
    });
  }

  // Nếu ít mục tiêu, dùng A* thường (chính xác hơn cho case đơn giản)
  if (goalPositions.length <= 1) {
    return aStarPathSolver(gameConfig, solutionConfig, blocklyConfig);
  }

  console.log(`TSP Solver: Phát hiện ${goalPositions.length} mục tiêu. Kích hoạt Meta-solver.`);

  // 2. Tìm thứ tự tối ưu
  let bestOrder: Position[] | null = null;
  
  // Hàm tính khoảng cách Manhattan
  const dist = (p1: Position, p2: Position) => Math.abs(p1.x - p2.x) + Math.abs(p1.y - p2.y) + Math.abs(p1.z - p2.z);

  if (goalPositions.length > 6) {
    // Heuristic: Nearest Neighbor
    const unvisited = [...goalPositions];
    ...
    while (unvisited.length > 0) {
      ...
      for (let i = 0; i < unvisited.length; i++) {
        const d = dist(currentPos, unvisited[i].pos);
        if (d < minDst) {
          minDst = d;
          nearestIdx = i;
        }
    ...
    // Heap's algorithm implementation for permutations
    const permute = (arr: any[]) => {
      const output: any[][] = [];
      const swap = (arrToSwap: any[], idx1: number, idx2: number) => {
    ...
    const allPermutations = permute(goalPositions);
    ...
    for (const p of allPermutations) {
      ...
      let curr = startPos;
      for (const item of p) {
        totalDist += dist(curr, item.pos);
        curr = item.pos;
      }
      totalDist += dist(curr, finishPos);
      
      if (totalDist < minTotalDist) {
        minTotalDist = totalDist;
        bestOrder = p.map(item => item.pos);
      }
    }
  }

  // 3. Thực thi A* theo từng chặng
  if (!bestOrder) return null;

  bestOrder.push(finishPos); // Thêm đích đến cuối cùng
  
  const fullRawActions: string[] = [];
  let currentSimState = new GameState(startPos, gameConfig.players[0].start.direction || 2, world);

  let prevTargetPos = startPos;
  
  for (const targetPos of bestOrder) {
    // Tạo cấu hình tạm thời cho bài toán con
    // Lưu ý: Chúng ta cần giữ nguyên orientation của player giữa các chặng
    const subGameConfig = {
      ...gameConfig,
      players: [{ start: { ...currentSimState.position, direction: currentSimState.direction } }],
      finish: targetPos
    };
    
    // Solution config con: chỉ cần đến đích (targetPos), không cần collect gì thêm (đã handle logic sequence)
    // Tuy nhiên aStarPathSolver hiện tại dựa vào heuristic mục tiêu.
    // Để đơn giản, ta tắt itemGoals trong bài toán con để nó chỉ tìm đường đi ngắn nhất.
    const subSolutionConfig = { ...solutionConfig, itemGoals: {} };

    // Gọi A*
    const subResult = aStarPathSolver(subGameConfig, subSolutionConfig, blocklyConfig);
    
    if (!subResult || !subResult.rawActions) {
      console.error("TSP Solver: Không tìm được đường đi giữa các điểm mục tiêu.");
      return null;
    }

    fullRawActions.push(...(subResult.rawActions as string[]));

    // Cập nhật trạng thái giả lập (vị trí, hướng) sau khi đến đích chặng
    // Việc này hơi trick vì aStarPathSolver không trả về final state.
    // Ta phải simulate lại hoặc tin tưởng vào targetPos.
    // Về hướng: aStarPathSolver kết thúc hướng nào? 
    // Ta cần tính toán hướng cuối cùng dựa trên các actions.
    for (const action of (subResult.rawActions as string[])) {
        if (action === 'turnLeft') currentSimState.direction = (currentSimState.direction + 3) % 4;
        else if (action === 'turnRight') currentSimState.direction = (currentSimState.direction + 1) % 4;
        else if (action === 'moveForward' || action === 'jump') {
             // Cập nhật vị trí (cơ bản)
             const d = directions[currentSimState.direction];
             currentSimState.position.x += d.x;
             currentSimState.position.z += d.z;
        }
    }
    
    // Thêm action tương tác tại điểm đến (nếu không phải là điểm finish cuối cùng thuần túy)
    // Kiểm tra xem tại targetPos có gì cần tương tác không
    const posKey = `${targetPos.x},${targetPos.y},${targetPos.z}`;
    if (world.collectiblesByPos.has(posKey)) {
        const item = world.collectiblesByPos.get(posKey)!;
        if (!currentSimState.collectedItems.has(item.id)) {
            fullRawActions.push('collect');
            currentSimState.collectedItems.add(item.id);
        }
    } else if (world.switchesByPos.has(posKey)) {
        const sw = world.switchesByPos.get(posKey)!;
        // Toggle switch
        fullRawActions.push('toggleSwitch');
  ...
  // 4. Tổng hợp Structure
  // Sử dụng lại createStructuredSolution
  // Chuyển đổi fullRawActions sang Action objects
  const structuredActions = convertRawToStructuredActions(fullRawActions);
  
  // Tự động detect Plowing Field ở đây nếu cần?
  // Để createStructuredSolution lo.
  
  const structuredSolution = createStructuredSolution(structuredActions, new Set(), solutionConfig, world);
  
  const optimalBlocks = calculateTotalBlocksInSolution(structuredSolution);
  const optimalLines = calculateOptimalLines(structuredSolution);

  return {
      optimalBlocks,
      optimalLines,
      rawActions: fullRawActions,
      structuredSolution,
      basicSolution: { main: structuredActions }
  };
};

/**
 * HÀM MỚI (PORTED FROM PYTHON): Detect pattern "Cày ruộng" (Plowing Field)
 * Nhận diện di chuyển ziczac trên lưới chữ nhật.
 */
const optimizePlowingField = (
    actions: Action[],
    solutionConfig: Solution
): { main: Action[]; procedures?: Record<string, Action[]> } | null => {
    // Heuristic đơn giản: Kiểm tra xem có phải là chuỗi lặp (move+collect) lớn không
    // Python version check coordinate grid. TS version check pattern actions.
    
    // Pattern cơ bản: Repeat(Move, Collect) -> Turn -> Repeat(Move, Collect) -> Turn...
    
    // 1. Phân tích actions thành các đoạn
    // Tìm chuỗi "move, collect" lặp lại nhiều nhất
    const { sequence, repeats, savings } = findLongestRepeatingSequence(actions);
    
    if (!sequence || repeats < 3) return null;
    
    // Check if sequence is exactly "move", "collect" (or similar atomic task)
    const isAtomicTask = sequence.length <= 2 && sequence.some(a => a.type === 'maze_collect');
    
    if (!isAtomicTask) return null;
    
    // Logic này phức tạp để implement generic đầy đủ ngay. 
    // Chúng ta sẽ implement simplified version: Nhận diện Nested Loop dựa trên Solution Config 'plowing_field'
    // Nếu config hint là plowing field, ta ép kiểu.
    
    // Tuy nhiên, để general, ta có thể dùng `optimizeWithVariablesAndLoops` đã có, 
    // hoặc cải tiến `optimizeWithLoops` để support nested tốt hơn.
    
    // Ở bản gameSolver.ts hiện tại, `optimizeWithLoops` đã support nested loop 
    // thông qua đệ quy: `optimizeRecursively`.
    // Nếu nó chưa nhận diện được plowing field, có thể do `findLongestRepeatingSequence` 
    // chưa bắt được pattern lớn (outer loop) chứa pattern nhỏ (inner loop).
    
    return null; // Tạm thời return null để fallback về logic loop thường, 
                 // vì optimizeWithLoops hiện tại đã khá mạnh.
};
 ...
 */
export const solveMaze = (gameConfig: GameConfig, solutionConfig: Solution, blocklyConfig?: QuestBlocklyConfig): Solution | null => {
  // Ưu tiên TSP Solver nếu có nhiều itemGoals
  const numGoals = Object.keys(solutionConfig.itemGoals || {}).length;
  // Check tổng số lượng items cụ thể
  let totalItems = 0;
  // Logic đếm items chính xác hơn có thể cần thiết, nhưng heuristic > 0 là đủ để kích hoạt check
  if (solutionConfig.itemGoals) {
      if (solutionConfig.itemGoals['switch']) totalItems += 1; // Coi như 1 nhóm
      // Các item khác...
  }
  
  // Nếu có vẻ là map phức tạp, thử TSP
  // (Lưu ý: aStarPathSolver gốc cũng handle được itemGoals nhưng không tối ưu thứ tự)
  // Để an toàn và tối ưu, ta luôn dùng TSP logic (nó sẽ fallback về A* nếu chỉ có 1 goal)
  // NHƯNG để tránh bug regression với các map đơn giản hiện tại, ta chỉ dùng TSP khi thực sự cần.
  
  if (solutionConfig.itemGoals && Object.keys(solutionConfig.itemGoals).length > 0) {
      // Dùng TSP wrapper
       return solveTSP(gameConfig, solutionConfig, blocklyConfig);
  }
...
const aStarPathSolver = (gameConfig: GameConfig, solutionConfig: Solution, blocklyConfig?: QuestBlocklyConfig): Solution | null => {
