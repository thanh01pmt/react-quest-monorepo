...
- [x] 3.1 Add `getSupabaseSubmissions(boardParticipantId)` to `SupabaseContestService.ts` — needed by `restoreChallengeStates` in `ContestContext`.
...
- [x] 3.3 Create `SupabaseProgressService.ts` to replace `FirestoreProgressService.ts`:
...
- [x] 3.4 Create `SupabaseSharedSessionService.ts` to replace `SharedSessionService.ts`:
...
- [x] 3.5 Update all callers of `FirestoreProgressService` and `SharedSessionService` to use new Supabase services.
- [x] 3.6 Verify RLS policies in Supabase allow anon access via RPC Gateway for contest operations.

## 4. Cleanup & Removal

- [x] 4.1 Delete `apps/react-quest-app/src/services/FirestoreProgressService.ts`.
- [x] 4.2 Delete `apps/react-quest-app/src/services/SharedSessionService.ts`.
- [x] 4.3 Delete `apps/react-quest-app/src/services/ContestService.ts`.
- [x] 4.4 Delete `apps/react-quest-app/src/config/firebase.ts`.
- [x] 4.5 Remove `VITE_FIREBASE_*` entries from `apps/react-quest-app/.env` and `.env.example`.
- [x] 4.6 Update `types/contest.ts` — change comment on `ContestParticipant.uid` from "Firebase Auth UID" to "Supabase Auth UID".
- [x] 4.7 **Final step only after all above tasks complete**: Uninstall `firebase` package from `apps/react-quest-app` (check `package.json` for usage before running `npm uninstall firebase`).
...
- [x] 5.8 Run TypeScript build: `npm run build` in `apps/react-quest-app` — zero Firebase-related errors.
- [x] 5.9 Run OpenSpec validation: `openspec validate migrate-contest-to-supabase-auth --strict`.
