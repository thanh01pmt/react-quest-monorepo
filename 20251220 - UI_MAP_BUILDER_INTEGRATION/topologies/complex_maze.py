...
class ComplexMazeTopology(BaseTopology):
    ...
    def generate_path_info(self, grid_size: tuple, params: dict) -> PathInfo:
        ...
        if path_coords_3d:
            current_segment = [path_coords_3d[0]]
            if len(path_coords_3d) > 1:
                # Determine initial direction
                last_diff = (
                    path_coords_3d[1][0] - path_coords_3d[0][0],
                    path_coords_3d[1][2] - path_coords_3d[0][2]
                )
                
                for i in range(1, len(path_coords_3d)):
                    curr = path_coords_3d[i]
                    prev = path_coords_3d[i-1]
        ...
        segment_analysis = {
            ...
            'types': ['maze_segment'] * len(segments),
        ...
        metadata = {
            "topology_type": "complex_maze_2d",
            "paths": [path_coords_3d],
            "dead_ends": dead_ends,
            "width": width,
            "depth": depth,
            # [NEW] Standard fields
            ...
            "semantic_positions": {
                ...
                'valid_pairs': [
                    {
                        'name': 'maze_simple_easy',
                        'start': 'start',
                        'end': 'end',
                        'path_type': 'segment_based',
                        'strategies': ['segment_based', 'junction_based'],
                        'difficulty': 'EASY',
                        'teaching_goal': 'Items at junctions'
                    },
                    {
                        'name': 'junction_emphasis_medium',
                        'start': 'start',
                        'end': 'end',
                        'path_type': 'junction_based',
                        'strategies': ['segment_based', 'junction_based'],
                        'difficulty': 'MEDIUM',
                        'teaching_goal': 'Items emphasize decision points'
                    },
                    {
                        'name': 'optimization_paths_hard',
                        'start': 'start',
                        'end': 'end',
                        'path_type': 'optimal_path',
                        'strategies': ['segment_based', 'path_optimization'],
                        'difficulty': 'HARD',
