...
class MapData:
    ...
    def __init__(self,
        ...
        self.metadata = metadata or {} # Khởi tạo metadata
        
        # [DEBUG] Log items received
        logger.info(f"MapData.__init__: Received {len(self.items)} items from placer")
    ...
    def to_game_engine_dict(self) -> Dict[str, Any]:
        ...
        logger.info(f"MapData.to_game_engine_dict: Processing {len(self.items)} items")
        
        for i, item in enumerate(self.items):
            item_pos = coord_to_obj(item['pos'], y_offset=1) # Item luôn ở y+1 so với nền
            item_type = item.get('type')
            
            # [FIX] Preserve solution-first metadata (pattern_id, segment_idx)
            extra_fields = {}
            if 'pattern_id' in item:
                extra_fields['pattern_id'] = item['pattern_id']
            if 'segment_idx' in item:
                extra_fields['segment_idx'] = item['segment_idx']
            ...
            if item_type in ['crystal', 'gem', 'key']:
                collectibles.append({
                    ...
                    "type": item_type, 
                    "position": item_pos,
                    **extra_fields  # Include pattern_id, segment_idx if present
                })
        ...
        logger.info(f"MapData.to_game_engine_dict: Created {len(collectibles)} collectibles, {len(interactibles)} interactibles")
        ...
        # Tự động tạo cấu hình toolbox dựa trên map_type và logic_type
        ...
        # 2. Blocks cho 3D (Nhảy)
        is_3d_map = self.map_type in [
            'staircase', 'staircase_3d', 'spiral_3d', 'stepped_island_clusters', 
            'hub_with_stepped_islands', 'complex_maze_3d'
        ]
        if is_3d_map:
            toolbox_contents.append({'kind': 'block', 'type': 'maze_jump'})

        # 3. Blocks cho Logic
        if self.logic_type == 'function_logic':
            toolbox_contents.append({'kind': 'block', 'type': 'maze_repeat'}) # Functions often use loops too
            toolbox_contents.append({'kind': 'category', 'custom': 'PROCEDURE', 'contents': []})
        elif self.logic_type == 'loop_logic' or self.logic_type == 'for_loop':
            toolbox_contents.append({'kind': 'block', 'type': 'maze_repeat'})
        ...
        # Override with preset if provided (legacy support)
