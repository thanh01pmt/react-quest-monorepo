    ...
    <div className="editorToolbar">
      <div className="toolbar-left">
        ...
        {currentEditor === 'blockly' && (
          <button onClick={onToggleSettings} className="settings-button" title={t('UI.SettingsTooltip', 'Settings')}>
      ...
      <div className="editor-switch-group">
        {supportedEditors.length > 1 ? (
          supportedEditors.map((editor) => (
            ...
            >
              {getEditorName(editor, t)}
            </button>
          ))
        ) : (
          <span className="editorLabel">{getEditorName(currentEditor, t)}</span>
        )}
      </div>
    </div>
  );
