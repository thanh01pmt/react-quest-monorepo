/**
 * Analyze Plus Shape Map - Ground Blocks Analysis
 * 
 * This script analyzes the plus_shape_islands_v1.json and generates a report.
 * 
 * Usage: npx tsx scripts/analyze-plus-map.ts
 */
...
const testDataPath = path.join(__dirname, 'test_data', 'plus_shape_islands_v1.json');
...
try {
  ...
  context.areas.forEach((area, i) => {
    ...
    if (area.subStructures) {
        area.subStructures.forEach(sub => {
            console.log(`    - SubStructure: ${sub.id} (${sub.type}): ${sub.coords.length} coords`);
        });
    }
     if (area.internalPaths) {
        area.internalPaths.forEach(sub => {
            console.log(`    - InternalPath: ${sub.id}: ${sub.points.length} points`);
        });
    }
  });
...
try {
  ...
  // Save report to file
  const reportPath = path.join(__dirname, 'output', 'plus_analysis_report.md');
